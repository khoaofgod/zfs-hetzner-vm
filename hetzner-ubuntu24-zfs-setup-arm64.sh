...
fully automatic script to install Ubuntu 24 with ZFS root on Hetzner ARM64 VPS
...
NOTE: This script is specifically designed for ARM64 architecture servers.
For x86_64 servers, use the regular hetzner-ubuntu24-zfs-setup.sh script.
end_header_info
...
ZBM_BIOS_URL="https://github.com/zbm-dev/zfsbootmenu/releases/download/v3.0.1/zfsbootmenu-release-aarch64-v3.0.1-linux6.1.tar.gz"
ZBM_EFI_URL="https://github.com/zbm-dev/zfsbootmenu/releases/download/v3.0.1/zfsbootmenu-release-aarch64-v3.0.1-linux6.1.EFI"

MAIN_BOOT="/main_boot"

# Ubuntu ARM64 ports repositories
MIRROR_SITE="http://ports.ubuntu.com"
MIRROR_MAIN="deb ${MIRROR_SITE}/ubuntu-ports/ ${UBUNTU_CODENAME} main restricted universe multiverse"
MIRROR_UPDATES="deb ${MIRROR_SITE}/ubuntu-ports/ ${UBUNTU_CODENAME}-updates main restricted universe multiverse"
MIRROR_BACKPORTS="deb ${MIRROR_SITE}/ubuntu-ports/ ${UBUNTU_CODENAME}-backports main restricted universe multiverse"
MIRROR_SECURITY="deb ${MIRROR_SITE}/ubuntu-ports/ ${UBUNTU_CODENAME}-security main restricted universe multiverse"
...
function bootstrap_ubuntu_system {
    ...
    # Add Ubuntu ports repository for ARM64 as trusted
    echo "deb [trusted=yes] http://ports.ubuntu.com/ubuntu-ports/ noble main" > /etc/apt/sources.list.d/ubuntu-temp.list
    ...
    mmdebstrap --variant=debootstrap \
      ...
      --include=systemd-resolved,locales,debconf-i18n,apt-utils,keyboard-configuration,console-setup,kbd,grub-efi-arm64,grub-efi-arm64-bin,initramfs-tools,zstd \
...
# Enable network services (will take effect after reboot)
systemctl enable systemd-networkd || echo "systemctl commands skipped in chroot"
systemctl enable systemd-resolved || echo "systemctl commands skipped in chroot"
EOF
}
...
function show_final_instructions {
    ...
    echo ""
    echo "=========================================="
    echo "  IMPORTANT BOOT NOTES:"
    echo "=========================================="
    echo ""
    if [ "$EFI_MODE" = true ]; then
        echo "🔧 If the system fails to boot:"
        echo "   1. Enter Hetzner Rescue Mode"
        echo "   2. Check if Secure Boot is disabled in BIOS/UEFI"
        ...
        echo "   2. Check MBR is properly installed"
        echo "   3. Verify ZFSBootMenu files exist in /boot partition"
    fi
    echo ""
    echo "=========================================="
    echo "Rebooting..."
